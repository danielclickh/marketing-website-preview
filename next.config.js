...
const strapiApiUrl =
  process.env.STRAPI_API_URL ?? 'https://cms.clickhouse.com:1338'
...
const nextConfig = {
  ...
  async rewrites() {
    return [
      ...
      {
        source: '/api/:path*',
        destination: 'https://cms.clickhouse.com:1338/api/:path*'
      },
      {
        source: '/uploads/:path*',
  ...
  async redirects() {
    return [
      ...
      },
      {
        source: '/slack',
        destination:
          'https://join.slack.com/t/clickhousedb/shared_invite/zt-21dqsxg8y-85z8Y1OZyHHluhN~d7_~ew',
        permanent: true
      },
      {
