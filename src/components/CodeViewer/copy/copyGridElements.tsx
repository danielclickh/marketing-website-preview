...
import { CellProps, SelectedRegion, SelectionFocus } from '@clickhouse/click-ui'
...
function copyTableAsMarkdown(table: HTMLTableElement) {
  ...
  bodyRows.forEach((row) => {
    const rowCells: string[] = Array.from(row.querySelectorAll('td, th')).map(
      (cell) => cell.textContent?.trim() ?? ''
    )
...
const copyGridElements = async ({
  ...
  if (windowSelection) {
    if (type === 'tsv') await navigator.clipboard.writeText(table.innerText)
    else await copyTableAsMarkdown(table)
