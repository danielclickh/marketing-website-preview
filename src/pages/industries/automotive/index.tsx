  ...
  return (
    <Layout footerData={footerData} seo={seo} headerData={headerData}>
      ...
      <section className='section-container my-16 lg:my-24'>
        <SuiTitle type='h2' className='mb-12 text-center'>
          Engineering power meets business outcomes
        </SuiTitle>
        <FeatureValueTable
          rows={[
            {
              feature:
                'High‑speed event ingestion (Kafka, Kinesis, Pub/Sub, etc.)',
              technical:
                'Ingest millions of vehicle or factory‑sensor events per second; dashboards update sub‑second',
              business:
                '**Warranty cost avoidance:** Detect anomalies early and perform predictive maintenance\n\n' +
                '**Revenue uplift:** Enable real‑time in‑car upsell / subscription sign up'
            },
            {
              feature: 'Fully parallelised query engine',
              technical:
                '<100ms response even under thousands of concurrent queries',
              business:
                '**Productivity gain:** Engineers stop waiting for queries to run, accelerating model iteration and feature rollout\n\n' +
                '**Better customer NPS:** Live telematics in apps without lag'
            },
            {
              feature: 'Materialised Views & async inserts',
              technical:
                'Roll‑ups and aggregations execute automatically upon ingest',
              business:
                '**Time‑to‑market:** Launch new analytics KPIs in days, not weeks\n\n' +
                '**Ops savings:** Eliminates separate ETL service licences'
            },
            {
              feature: 'ClickPipes (turn‑key ingestion in ClickHouse Cloud)',
              technical: 'One‑click pipelines replace bespoke ETL',
              business:
                '**TCO:** Reduce costs significantly by removing the need for third‑party ingestion tools\n\n' +
                '**Faster PoCs:** Start ingesting data in <15min, shortening sales cycles'
            },
            {
              feature: 'Native OpenTelemetry support',
              technical: 'Logs, traces, metrics unified in one store',
              business:
                '**Reduce MTTR:** Cross‑team troubleshooting can shrink from hours to minutes, reducing downtime costs\n\n' +
                '**Compliance evidence:** Single audit trail for OTA updates'
            },
            {
              feature: 'Best‑in‑class compression & columnar storage',
              technical: 'Up to 10× smaller footprint vs. row stores',
              business:
                '**TCO:** Storage spend reduced by 60+%\n\n' +
                '**Greener IT:** Lower energy footprint supports ESG goals'
            },
            {
              feature: 'Elastic horizontal/vertical scaling',
              technical: 'Auto‑scales during peaks (e.g., model launch)',
              business:
                '**CapEx/Opex optimisation:** Pay only for peak when needed, no over‑provisioning\n\n' +
                '**Risk mitigation:** SLA adherence even under traffic spikes'
            },
            {
              feature:
                'Enterprise security & compliance (CMEK, BYOC, GDPR TTL)',
              technical: 'Data retains sovereignty; fine‑grained RBAC',
              business:
                '**Regulatory peace‑of‑mind:** Avoid GDPR fines (up to 4% global turnover)\n\n' +
                '**Deal acceleration:** Skip lengthy security review'
            },
            {
              feature: 'REST & gRPC APIs plus rich ecosystem',
              technical: 'Build apps directly on ClickHouse',
              business:
                '**New revenue streams:** Faster rollout of connected‑car analytics apps\n\n' +
                '**Developer efficiency:** Smaller codebase, lower maintenance cost'
            }
          ]}
        />
      </section>
...
function FeatureValueTable({
  ...
  return (
    <table className='w-full text-left'>
      <thead>
        <tr>
          <th className='hidden border-b border-neutral-700 py-6 pr-8 text-xl font-semibold lg:table-cell xl:pr-16'>
            Feature
          </th>
          <th className='w-[30%] border-b border-neutral-700 py-6 pr-6 text-xl font-semibold'>
            Technical value
          </th>
          <th className='w-[40%] border-b border-neutral-700 px-6 py-6 text-xl font-semibold'>
            Business value
          </th>
        </tr>
      </thead>
      <tbody>
        {rows.map(({ feature, technical, business }, rowIndex) => {
          return (
            <tr className='min-h-12' key={rowIndex}>
              ...
              <td
                valign='top'
                className='space-y-4 border-b border-neutral-700 py-4 pr-6 text-neutral-200 lg:pr-8'>
                {typeof technical === 'string' ? (
                  <Markdown encloseByDiv={false}>{technical}</Markdown>
                ) : (
                  technical
                )}
              </td>
              <td
                valign='top'
                className='space-y-4 border-b border-neutral-700 px-6 py-4 text-neutral-200'>
                {typeof business === 'string' ? (
                  <Markdown encloseByDiv={false}>{business}</Markdown>
                ) : (
                  business
                )}
              </td>
