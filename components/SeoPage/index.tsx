  ...
  return (
    <main>
      ...
      <div className='lg:flex flex-row items-start gap-6'>
        ...
        <aside className='w-full lg:max-w-sm flex-shrink-0 flex-grow-0 align-top'>
          {moreLikeThis?.length > 0 && (
            <div className='mb-8'>
              <h3 className='text-lg font-bold mb-6'>More like this</h3>
              {moreLikeThis.map(item => (
                <Link href={item.link} className='block w-full mb-4'>
                  <SuiButton
                    type='empty'
                    color='primary'
                    className='font-base border border-neutral-700 hover:translate-y-0 hover:border-primary-400/40 block w-full hover:no-underline'>
                    <div className='flex flex-row gap-6 items-center w-full'>
                      <div className='text-left flex-grow flex-shrink basis-0'>
                        {item.title}
                      </div>
          ...
          <SuiPanel color='bg-c4/10' padding='lg' isRounded className='mt-6'>
            <h3 className='text-lg font-bold mb-2'>{newsLetterData.title}</h3>
            <SuiText size='sm' weight='medium' color='secondary' className='mb-6'>
